...
> **Scope（范围）**：给出 match 算法的整体设计、特征含义和匹配模式，帮助 AI 形成正确的心智模型。
> **Contract（契约）**：描述特征、模式和重要不变量（例如 IPID 作为必要条件），但不承诺在此维护精确权重或阈值的“第二份拷贝”。
> **Implementation Pointers**：需要精确逻辑时，请阅读 `capmaster/core/connection/` 下的 matcher、builder 等实现，以及 `tests/` 中的匹配相关用例。
> **Maintenance**：当匹配算法语义发生变化（新增/移除特征、改变判定规则）时更新此文档；具体权重和细节以代码和测试为准。
