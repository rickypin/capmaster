...
@register_module
class JsonStatsModule(AnalysisModule):
    """Generate JSON statistics.
    
    Analyzes JSON messages in HTTP traffic to extract:
    - JSON message counts and sizes
    - HTTP methods and status codes for JSON APIs
    - Request/response patterns
    - Connection endpoints
    
    This helps identify JSON API errors and performance issues.
    """
    ...
    def build_tshark_args(self, input_file: Path) -> list[str]:
        ...
        Returns:
            List of tshark arguments for JSON statistics extraction
        """
        return [
            "-Y",
            "json",
    ...
    def post_process(self, tshark_output: str, output_format: str = "txt") -> str:
        """
        Post-process JSON messages to generate statistics.
        
        Groups JSON messages by:
        ...
        Returns:
            Formatted output with JSON statistics
        """
        if not tshark_output.strip():
            return "No JSON messages found\n"
        ...
        lines.append("-" * 80)
        lines.append(f"  Total JSON Messages:     {total_messages}")
        lines.append(f"  JSON Requests:           {request_count}")
        lines.append(f"  JSON Responses:          {response_count}")
        if total_messages > 0:
        ...
        # HTTP Methods
        if method_counter:
            lines.append("HTTP Methods (JSON Requests):")
            lines.append("-" * 80)
        ...
        # HTTP Response Codes
        if response_counter:
            lines.append("HTTP Response Codes (JSON Responses):")
            lines.append("-" * 80)
        ...
        if content_type_counter:
            ...
            lines.append("-" * 80)
            lines.append(f"{'Content Type':<50} {'Count':>10}")
            lines.append("-" * 80)
            ...
            for ctype, count in sorted(content_type_counter.items(), key=lambda x: -x[1]):
                ...
                lines.append(f"{ctype_display:<50} {count:>10}")
            lines.append("")
