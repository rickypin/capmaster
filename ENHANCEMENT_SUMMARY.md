...
通过添加**双维度基数分析**（端点基数 + 端口复用），显著增强了 `--endpoint-stats` 功能的准确性和适用性，特别是在以下场景：
...
| 场景 | 端点基数 | 端口复用 | 最终置信度 |
|------|---------|---------|-----------|
| 大型Web服务 | HIGH | HIGH | **HIGH** (双重确认) |
| 负载均衡集群 | MEDIUM | HIGH | **HIGH** (端口复用提升) |
| 单服务器 | HIGH | LOW | **HIGH** (基数足够) |
| 小型集群 | MEDIUM | MEDIUM | **MEDIUM** (双重支持) |

这是对现有多层检测体系的重要补充，利用了客户端-服务端架构的两个本质特征，使服务端识别更加智能、可靠和适应现代网络架构。
